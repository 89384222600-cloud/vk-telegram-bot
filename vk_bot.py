...
def init_bot():
    saved = get_saved_id()
    if saved is None:
        print("⚙️ Первый запуск. Инициализация...")
        params = {"owner_id": VK_GROUP_ID, "count": 1, "access_token": VK_TOKEN, "v": "5.131"}
        r = requests.get(VK_WALL, params=params).json()
        if "response" in r and r["response"]["items"]:
            latest_id = r["response"]["items"][0]["id"]
            save_id(latest_id)
            print(f"✅ Инициализация завершена. ID: {latest_id}")
            return True
    return False
